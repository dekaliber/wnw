...
const en = {
  ...
  settings: 'Settings',
  questionsSetting: 'Questions per game',
  timers: 'Timers',
  ...
  startWithPractice: (n: number) => `Start practice + ${n} questions`,
  uploadCsv: 'Use your own questions (CSV)',
  csvColumnsHint: 'Columns: id, category, format, text, answer. Rows with category “Practice” are used as the practice question.',
  customLoaded: (n: number) => `${n} question${n === 1 ? '' : 's'} loaded`,
  customPracticeLoaded: (n: number) => ` + ${n} practice`,
  customFrom: (file: string) => `from ${file}`,
  replaceCsv: 'Upload new CSV',
  useBuiltIn: 'Reset to built-in questions',
  csvCouldNotLoad: (file: string) => `Couldn’t load ${file}`,
  csvRow: (row: number) => `Row ${row}`,
  csvMore: (n: number) => `…and ${n} more`,
  csvSkipped: (n: number) => `${n} row${n === 1 ? '' : 's'} skipped`,
  csvSkippedHint: 'Fix them and tap Upload new CSV, or go ahead with the rest.',
  bringBackPlayed: 'Bring back played questions',
  playedRecently: 'Reset recently played questions',
  restingHint: (n: number) =>
    `${n} question${n === 1 ? ' was' : 's were'} played in the last 24 hours, so ${n === 1 ? 'it’s' : 'they’re'} being held back to avoid repeats. If that was just a test run, bring ${n === 1 ? 'it' : 'them'} back.`,
  broughtBack: '✓ Every question is available again.',
  noneResting: 'No questions have been played in the last 24 hours, so there’s nothing to bring back.',
  shuffleQuestions: 'Shuffle the order',
  additionalOptions: 'Additional options',
  languageSetting: 'Language',
  languageHint: 'Just for this phone — everyone picks their own.',
  done: 'Done',
  shuffleHint: 'Off plays them top to bottom, in file order.',
  ...
  practiceNoScore: 'Doesn’t count',
  startForReal: 'Start question 1',
...
const fr: typeof en = {
  ...
  settings: 'Réglages',
  questionsSetting: 'Questions par partie',
  timers: 'Minuteurs',
  ...
  startWithPractice: (n: number) => `Lancer l’essai + ${n} questions`,
  uploadCsv: 'Utiliser vos propres questions (CSV)',
  csvColumnsHint: 'Colonnes : id, category, format, text, answer. Les lignes de catégorie « Practice » servent de question d’essai.',
  customLoaded: (n: number) => `${n} question${n === 1 ? '' : 's'} chargée${n === 1 ? '' : 's'}`,
  customPracticeLoaded: (n: number) => ` + ${n} d’essai`,
  customFrom: (file: string) => `depuis ${file}`,
  replaceCsv: 'Importer un nouveau CSV',
  useBuiltIn: 'Revenir aux questions intégrées',
  csvCouldNotLoad: (file: string) => `Impossible de charger ${file}`,
  csvRow: (row: number) => `Ligne ${row}`,
  csvMore: (n: number) => `…et ${n} de plus`,
  csvSkipped: (n: number) => `${n} ligne${n === 1 ? ' ignorée' : 's ignorées'}`,
  csvSkippedHint: 'Corrigez-les puis appuyez sur Importer un nouveau CSV, ou continuez avec le reste.',
  bringBackPlayed: 'Réutiliser les questions déjà jouées',
  playedRecently: 'Réinitialiser les questions déjà jouées',
  restingHint: (n: number) =>
    n === 1
      ? '1 question a été jouée ces dernières 24 heures, elle est donc mise de côté pour éviter les répétitions. Si c’était juste un essai, réutilisez-la.'
      : `${n} questions ont été jouées ces dernières 24 heures, elles sont donc mises de côté pour éviter les répétitions. Si c’était juste un essai, réutilisez-les.`,
  broughtBack: '✓ Toutes les questions sont de nouveau disponibles.',
  noneResting: 'Aucune question n’a été jouée ces dernières 24 heures, il n’y a donc rien à réutiliser.',
  shuffleQuestions: 'Mélanger l’ordre',
  additionalOptions: 'Autres options',
  languageSetting: 'Langue',
  languageHint: 'Pour ce téléphone seulement — chacun choisit la sienne.',
  done: 'Terminé',
  shuffleHint: 'Désactivé, elles passent dans l’ordre du fichier.',
  ...
  practiceQuestion: 'Question d’essai',
  practiceBadge: 'Essai',
  practiceNoScore: 'Ne compte pas',
  startForReal: 'Lancer la question 1',
